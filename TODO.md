...
- [ ] `unsuable` → `unusable`, twice: `server/client.go:54` and `internal/protocol/conn.go:23`. The
      second one is in `ErrUnrecoverable`'s message, which reaches clients in an error frame.
- [ ] `server/commands.go:23` — the comment says the table is "Populated in `init()`". It is a
