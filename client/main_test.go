...
// The happy path: the client sends the cleaned name and adopts whatever name
// the server acknowledges, which may differ from what the user typed.
func TestSetUserNameReturnsTheAckedName(t *testing.T) {
	...
	go func() {
		...
		pipe.Peer.SendHandshakeAck("alice_2") // server renamed a duplicate
	}()
